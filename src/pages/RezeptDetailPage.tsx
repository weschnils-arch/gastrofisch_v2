  ...
  if (!recipe) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 bg-white">
        ...
        <h2 className="font-playfair text-3xl font-semibold text-graphite">Rezept nicht gefunden</h2>
        <Link to="/rezepte" className="btn-primary inline-flex items-center gap-2">
          <ArrowLeft size={18} /> Zurück zu den Rezepten
        </Link>
      </div>
    );
  }
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="relative h-[55vh] md:h-[65vh] overflow-hidden">
        ...
        <div className="absolute bottom-8 left-6 right-6 md:left-12 md:right-12 text-white">
          ...
          <p className="font-lato text-base md:text-lg text-white/80 mb-5 max-w-2xl">{recipe.description}</p>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-20">
            ...
            <div className="sticky top-24">
              <div className="border border-adria/10 rounded-2xl p-6 md:p-8 bg-adria/3 shadow-lg">
                <div className="flex flex-col gap-4 mb-6 pb-4 border-b border-adria/10">
                  <h2 className="font-playfair text-2xl font-semibold text-adria">Zutaten</h2>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mt-12 p-6 bg-adria/5 border border-adria/15 rounded-2xl">
              <p className="font-lato text-sm text-adria font-semibold uppercase tracking-wider mb-2">Tipp vom Küchenchef</p>
              <p className="font-lato text-graphite/70 leading-relaxed">
                Für dieses Rezept empfehlen wir die Verwendung von frischen Zutaten aus unserer Fisch-Boutique in der Zollergasse 12,{' '}
                <Link to="/einzelhandel" className="text-adria hover:underline font-medium">Wien-Neubau</Link>.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {related.map((r) => (
                ...
                >
                  ...
                  <div className="p-5">
                    <h3 className="font-playfair text-lg font-semibold text-graphite mb-1 group-hover:text-adria transition-colors">{r.title}</h3>
                    <p className="font-lato text-sm text-graphite/60 line-clamp-2">{r.description}</p>
                    <div className="flex items-center gap-3 mt-3 text-xs text-graphite/50 font-lato">
                      ...
                      <span className={`px-2 py-0.5 rounded-full font-bold uppercase ${getDifficultyColor(r.difficulty)}`}>{r.difficulty}</span>
                    </div>
                  </div>
                </Link>
              ))}
